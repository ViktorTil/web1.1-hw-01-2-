...
def extract_archive(archive_path, target_folder, messenger):
    if archive_path.endswith('.zip'):
        ...
        messenger.send_message(f"Unsupported archive format: {archive_path}")

def organize_files(folder, messenger):
    ...
    for root, _, files in os.walk(folder):
        for filename in files:
            if filename.endswith('.zip') or filename.endswith('.tar.gz') or filename.endswith('.tar') or filename.endswith('.rar'):
                ...
                try:
                    extract_archive(archive_path, extract_folder, messenger)
                    ...
                    messenger.send_message(
                        "Can not proceed with unpacking. Archive is corrupted.")
